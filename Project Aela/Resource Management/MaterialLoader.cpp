	...
	if (in.is_open()) {
		while (std::getline(in, line)) {
			if (line.find("newmtl ") != std::string::npos) {
				...
				std::string fileName = line.substr(7, line.size() - line.find(" ") - 1);
				GLuint texture = loadDDSToGLuint("How do I get the folder path?" + fileName);
			} else {
...
GLuint Aela::MaterialLoader::loadDDSToGLuint(std::string filePath) {
